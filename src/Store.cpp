...
bool Store::Add(string const& barcode,
                string const& name,
  ...
  try {
    if (zzz->MoveToElementByPath(id, path("/items/barcode/" + barcode),
                                 GLOBAL) == false) {
      double addQuantity = ToDouble(quantity);
      double addPrice = ToDouble(price);
      double newPrice = addQuantity * addPrice;

      zzz->AddElementByPath(
          id, path("/items/barcode/" + barcode + "/name/" + name), GLOBAL);
      zzz->AddElementByPath(
          id, path("/items/barcode/" + barcode + "/measure/" + measure),
          GLOBAL);
      zzz->AddElementByPath(
          id, path("/items/barcode/" + barcode + "/quantity/" + quantity),
          GLOBAL);
      zzz->AddElementByPath(
          id,
          path("/items/barcode/" + barcode + "/price/" + ToString(newPrice)),
          GLOBAL);
    } else {
      if (zzz->GetElementByPath(
                 id, path("/items/barcode/" + barcode + "/name/"), GLOBAL)
              .compare(name) != 0) {
        if (zzz->MoveToElementByPath(
                id, path("/items/barcode/" + barcode + "/name"), GLOBAL)) {
      ...
      double oldQuantity = ToDouble(zzz->GetElementByPath(
          id, path("/items/barcode/" + barcode + "/quantity/"), GLOBAL));
      double addQuantity = ToDouble(quantity);
      double oldPrice = ToDouble(zzz->GetElementByPath(
          id, path("/items/barcode/" + barcode + "/price/"), GLOBAL));
      double addPrice = ToDouble(price);

      double newQuantity = oldQuantity + addQuantity;
      double newPrice = oldPrice + addQuantity * addPrice;

      setValueByPath(id, "/items/barcode/" + barcode + "/quantity/",
                     newQuantity);
...
bool Store::Get(string const& barcode, string const& quantity) {
  bool result = false;
  ...
  try {
    if (barcode.compare("all") == 0) {
      ...
      if (zzz->MoveToElementByPath(id, path("/items/barcode/" + barcode + "/"),
        ...
        printf("%s, %s, %s, %0.2lf, %0.2lf\n", barcode.c_str(), name.c_str(),
               measure.c_str(), quantityValue, priceValue);
...
void Store::setValueByPath(long long int id,
                           std::string const& dividerPath,
                           double const value) {
